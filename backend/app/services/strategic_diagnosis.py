...
def strategic_diagnosis(analysis: Dict) -> Dict:
    ...
    insufficient_data = (
        proof_gaps >= 4
        or (legal_strength == 0 and proof_gaps >= 2)
    ...
    if insufficient_data:
        probability = None
        complexity = "preliminar"
        financial_risk = "indeterminado"
        strong_points = [
            "Não há base factual mínima suficiente para apontar força material ou prognóstico com segurança."
        ]
        positioning = (
            "Caso em fase preliminar, sem base informacional suficiente para posicionamento estratégico conclusivo."
        )
        negotiation_opportunity = (
            "Qualquer tentativa de negociação ou pressão estratégica depende antes da consolidação mínima dos fatos, "
            "documentos e recorte do pedido."
        )
        recommended_strategy = (
            "Interromper qualquer prognóstico de mérito neste estágio e priorizar complementação factual e documental "
            "antes de decidir por negociação ou ajuizamento."
        )
        risk_alerts = (
            "Há insuficiência relevante de dados. Emitir percentual de êxito, tese material fechada ou prazo estimado "
            "neste estágio seria indevido."
        )
        execution_plan = [
            "Levantar cronologia objetiva dos fatos com datas minimamente verificáveis.",
            "Consolidar documentos essenciais e identificar exatamente quais provas ainda não existem.",
            "Definir com precisão o núcleo do pedido e a base jurídica realmente sustentável.",
            "Somente após a complementação mínima reprocessar a análise estratégica e de viabilidade.",
        ]
    else:
        if risk_level == "low":
            complexity = "baixa"
